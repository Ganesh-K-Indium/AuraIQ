...
export const CypherStudio: React.FC = () => {
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      {/* Header Banner */}
      <div className="glass-panel p-5 rounded-2xl bg-gradient-to-r from-amber-950/40 via-slate-900/60 to-slate-950 border border-amber-900/30 flex items-center justify-between">
        <div>
        ...
        <button
          ...
          disabled={running}
          className="flex items-center space-x-2 px-5 py-2.5 rounded-xl bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold text-xs shadow-lg shadow-amber-500/20 transition-all active:scale-95"
        >
              ...
              return (
                <div
                  ...
                  className={`p-3.5 rounded-xl border transition-all cursor-pointer ${
                    isSelected
                      ? "glass-panel border-amber-500/60 bg-amber-500/10 shadow-lg shadow-amber-500/5"
                      : "bg-slate-900/80 border-slate-800 hover:border-slate-700 text-slate-300"
                  }`}
                >
                  <div className="flex items-center justify-between">
                    <span className="font-semibold text-xs text-white">{tpl.title}</span>
                    <span className="px-2 py-0.5 text-[9px] font-semibold bg-slate-800 text-amber-300 border border-slate-700 rounded">
                      {tpl.category}
                    </span>
                  </div>
                  <pre className="font-mono text-[10px] text-slate-400 mt-2 truncate">
                    {tpl.cypher.split("\n")[0]}
                  </pre>
                </div>
              );
        ...
        <div className="lg:col-span-8 space-y-4">
          ...
          <div className="glass-panel p-5 rounded-2xl border border-slate-800 space-y-3">
            <div className="flex items-center justify-between text-xs text-slate-400 border-b border-slate-800 pb-2">
              <span className="flex items-center space-x-1.5 font-mono text-slate-300">
                <FileCode className="w-3.5 h-3.5 text-amber-400" />
                <span>Cypher Editor (Bolt: 7687)</span>
              </span>
              <span className="text-[11px] font-mono text-slate-500">Read-Only Guard Enabled</span>
            </div>
            ...
            <textarea
              ...
              rows={6}
              className="w-full p-3.5 rounded-xl bg-slate-950 border border-slate-800 font-mono text-xs text-emerald-300 focus:outline-none focus:border-amber-500/70 shadow-inner"
            />
          </div>
          ...
          <div className="glass-panel rounded-2xl border border-slate-800 overflow-hidden shadow-2xl">
            ...
            {queryResult?.rows && queryResult.rows.length > 0 ? (
                    ...
                    {queryResult.rows.map((row: any, idx: number) => (
                      <tr key={idx} className="hover:bg-slate-800/30 transition-colors">
                        {queryResult.columns.map((col: string) => {
                          ...
                          return (
                            <td key={col} className="px-4 py-2.5 font-mono text-slate-200">
                              {formatted}
                            </td>
                          );
                        })}
              ...
              <div className="p-6 text-xs text-red-400 bg-red-950/20 font-mono">
                Error: {queryResult.error}
              </div>
            ) : (
              <div className="p-12 text-center text-xs text-slate-500 font-mono">
                Click "Execute Cypher" to run the query against Neo4j.
              </div>
            )}
          </div>
        </div>
