...
export const ExecutiveDashboard: React.FC<ExecutiveDashboardProps> = ({
  ...
  if (!clientData) {
    return (
      <div className="flex items-center justify-center p-16 text-slate-500 dark:text-slate-400 text-sm">
  ...
  return (
    <div className="space-y-6 animate-fadeIn font-sans">
      {/* Header Banner */}
      <div className="glass-panel p-6 rounded-2xl bg-gradient-to-r from-slate-50 via-white to-slate-50 dark:from-slate-900/90 dark:via-[#0d1322] dark:to-slate-950 border border-slate-200 dark:border-slate-800 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6 shadow-md">
        <div>
          <div className="flex items-center space-x-3">
            <h1 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">{clientData.name}</h1>
            <span className="px-2.5 py-0.5 text-xs font-semibold bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border border-emerald-500/30 rounded-full">
              {clientData.net_worth_tier}
            </span>
            <span className="px-2.5 py-0.5 text-xs font-medium bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 rounded-full font-mono">
              {clientData.tax_residence}
            </span>
          </div>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 flex items-center space-x-2">
            <span>Client ID: <span className="font-mono text-slate-700 dark:text-slate-300">{clientData.client_id}</span></span>
            <span>•</span>
            <span>Investment Mandate: <span className="text-emerald-600 dark:text-emerald-400 font-semibold">{clientData.risk_profile.tolerance_level}</span></span>
          </p>
        </div>
        ...
        <div className="flex items-center space-x-3">
          ...
          >
            ...
            <span className="text-white">Open AI Copilot</span>
          </button>
          <button
            onClick={onNavigateToGraph}
            className="flex items-center space-x-2 px-4 py-2.5 rounded-xl bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 font-semibold text-xs border border-slate-200 dark:border-slate-700 transition-all shadow-sm"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="glass-panel p-5 rounded-2xl border border-slate-200 dark:border-slate-800/80 hover:border-emerald-500/40 transition-all bg-white dark:bg-slate-900/60 shadow-sm">
        ...
        <div className="glass-panel p-5 rounded-2xl border border-slate-200 dark:border-slate-800/80 hover:border-purple-500/40 transition-all bg-white dark:bg-slate-900/60 shadow-sm">
          <div className="flex items-center justify-between text-slate-500 dark:text-slate-400 mb-2">
        ...
        <div className="glass-panel p-5 rounded-2xl border border-slate-200 dark:border-slate-800/80 hover:border-blue-500/40 transition-all bg-white dark:bg-slate-900/60 shadow-sm">
          <div className="flex items-center justify-between text-slate-500 dark:text-slate-400 mb-2">
            <span className="text-[11px] font-bold uppercase tracking-wider">Supervisory Standard</span>
            <CheckCircle2 className="w-4 h-4 text-blue-600 dark:text-blue-400" />
          </div>
          <div className="text-lg font-bold text-blue-700 dark:text-blue-300">SEC Reg BI & MiFID II</div>
          <div className="text-[11px] text-emerald-600 dark:text-emerald-400 mt-1 font-medium">
            Status: Fully Compliant Mandate
          </div>
        </div>

        <div className="glass-panel p-5 rounded-2xl border border-slate-200 dark:border-slate-800/80 hover:border-amber-500/40 transition-all bg-white dark:bg-slate-900/60 shadow-sm">
          <div className="flex items-center justify-between text-slate-500 dark:text-slate-400 mb-2">
            <span className="text-[11px] font-bold uppercase tracking-wider">Estate & Trust Structure</span>
            <Building2 className="w-4 h-4 text-amber-600 dark:text-amber-400" />
          </div>
          <div className="text-base font-bold text-amber-700 dark:text-amber-300 truncate">
      ...
      <div className="glass-panel p-6 rounded-2xl border border-slate-200 dark:border-slate-800 space-y-5 bg-white dark:bg-slate-900/60 shadow-md">
        <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800/80 pb-3">
          ...
          <button
            ...
            disabled={simulating}
            className="flex items-center space-x-2 px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs shadow-lg shadow-emerald-600/20 transition-all active:scale-95"
          >
            <RefreshCw className={`w-3.5 h-3.5 text-white ${simulating ? "animate-spin" : ""}`} />
            <span className="text-white">{simulating ? "Calculating Orders..." : "Run Rebalance Simulation"}</span>
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="space-y-2">
            <div className="flex items-center justify-between text-xs">
              <span className="font-semibold text-slate-700 dark:text-slate-300">Target Equities Weight</span>
              <span className="font-mono font-bold text-emerald-600 dark:text-emerald-400">{targetEquity}%</span>
            </div>
            <input
              type="range"
              min={10}
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between text-xs">
              <span className="font-semibold text-slate-700 dark:text-slate-300">Target Fixed Income Weight</span>
              <span className="font-mono font-bold text-blue-600 dark:text-blue-400">{targetFixedIncome}%</span>
            </div>
            <input
              type="range"
              min={10}
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between text-xs">
              <span className="font-semibold text-slate-700 dark:text-slate-300">Target Alternatives Weight</span>
              <span className="font-mono font-bold text-pink-600 dark:text-pink-400">{targetAlternatives}%</span>
            </div>
            <input
              type="range"
              min={0}
        ...
        {simulationResult && (
          <div className="mt-4 p-4 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 space-y-3 animate-fadeIn shadow-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {simulationResult.trades?.map((t: any, idx: number) => (
                <div key={idx} className="p-3 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/80 text-xs shadow-sm">
                  <div className="flex items-center justify-between mb-1">
                    <span
                      className={`font-mono font-bold text-[10px] px-1.5 py-0.5 rounded ${
                        t.action === "BUY" ? "bg-emerald-500/10 text-emerald-700 dark:text-emerald-400" : "bg-red-500/10 text-red-700 dark:text-red-400"
                      }`}
                    >
                      {t.action}
                    </span>
                  ...
                  <div className="text-[11px] text-slate-700 dark:text-slate-300 font-semibold">{t.asset_class}</div>
                  <div className="text-[10px] text-slate-500 dark:text-slate-400 truncate">{t.suggested_instrument}</div>
                </div>
              ))}
            </div>

            <div className="text-xs text-slate-600 dark:text-slate-400 pt-2 border-t border-slate-200 dark:border-slate-900">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="glass-panel p-6 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 shadow-md">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center space-x-2">
              <PieIcon className="w-4 h-4 text-emerald-600 dark:text-emerald-400" />
              <h3 className="text-sm font-bold text-slate-900 dark:text-white">FIBO Asset Class Breakdown</h3>
            </div>
            <span className="text-xs text-slate-500 dark:text-slate-400 font-mono">{primaryPortfolio?.portfolio_id}</span>
          </div>
          ...
          <div className="grid grid-cols-3 gap-2 mt-2 pt-3 border-t border-slate-200 dark:border-slate-800">
            {pieData.map((item, idx) => (
              <div key={item.name} className="flex items-center space-x-2 text-xs">
                <span className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: COLORS[idx % COLORS.length] }} />
                <span className="text-slate-600 dark:text-slate-300">{item.name}: <span className="font-semibold text-slate-900 dark:text-white">{item.pct}%</span></span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="glass-panel p-6 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 shadow-md">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center space-x-2">
              ...
              <h3 className="text-sm font-bold text-slate-900 dark:text-white">Sector Concentration ($K USD)</h3>
            </div>
            <span className="text-xs text-amber-600 dark:text-amber-400 font-semibold">Overweight Screened</span>
          </div>
          ...
          <div className="mt-2 pt-3 border-t border-slate-200 dark:border-slate-800 text-xs text-slate-500 dark:text-slate-400 flex items-center justify-between">
            ...
            <span className="font-mono text-slate-700 dark:text-slate-300 font-semibold">Technology &gt; 50%</span>
          </div>
        </div>
      </div>
      ...
      <div className="glass-panel rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-md bg-white dark:bg-slate-900/60">
        <div className="p-5 border-b border-slate-200 dark:border-slate-800/80 flex items-center justify-between bg-slate-50 dark:bg-slate-950/40">
          <div>
            <h3 className="text-sm font-bold text-slate-900 dark:text-white">Underlying FIBO Financial Instruments</h3>
            <p className="text-xs text-slate-500 dark:text-slate-400">Directly mapped to Neo4j graph nodes and holdings</p>
          </div>
          <span className="text-xs font-mono bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 px-3 py-1 rounded-lg border border-slate-200 dark:border-slate-700">
              ...
              {primaryPortfolio?.holdings?.map((h, idx) => (
                <tr key={idx} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                  <td className="px-5 py-3.5">
                    ...
                    <div className="font-mono text-[11px] text-slate-500 dark:text-slate-400">{h.ticker_or_id}</div>
                  </td>
                  <td className="px-5 py-3.5">
                  ...
                  <td className="px-5 py-3.5 text-slate-600 dark:text-slate-300">{h.sector || "N/A"}</td>
                  ...
                  <td className="px-5 py-3.5 text-right font-mono text-slate-900 dark:text-white font-semibold">
