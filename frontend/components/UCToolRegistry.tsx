...
export const UCToolRegistry: React.FC = () => {
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      {/* Banner */}
      <div className="glass-panel p-6 rounded-2xl bg-gradient-to-r from-amber-950/40 via-slate-900/60 to-slate-950 border border-amber-900/30 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Standardized Python & SQL functions registered under <code className="font-mono text-amber-300">wealth_mgmt_catalog.fibo_knowledge_graph</code>
          </p>
        </div>
      </div>
              ...
              return (
                <div
                  ...
                  className={`p-4 rounded-xl border transition-all cursor-pointer ${
                    isSelected
                      ? "glass-panel border-amber-500/50 bg-amber-500/10 shadow-lg shadow-amber-500/5"
                      : "bg-slate-900/80 border-slate-800 hover:border-slate-700 text-slate-300"
                  }`}
                >
                  <div className="flex items-center justify-between">
                    <span className="font-mono text-xs font-bold text-white">{tool.name}</span>
                    <span className="px-2 py-0.5 text-[9px] font-semibold bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 rounded">
                      UC Tool
                    </span>
                  </div>
                  <p className="text-xs text-slate-400 mt-1 line-clamp-2">{tool.description}</p>
                  <div className="font-mono text-[10px] text-amber-400/80 mt-2 truncate">
                    {tool.catalog_path}
                  </div>
                </div>
              );
          ...
          <div className="glass-panel p-6 rounded-2xl border border-slate-800 space-y-4">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <div>
                ...
                <p className="text-xs text-slate-400 font-mono text-[11px]">{selectedTool.catalog_path}</p>
              </div>
              <button
            ...
            <div>
              ...
              <textarea
                ...
                rows={4}
                className="w-full p-3 rounded-lg bg-slate-950 border border-slate-800 font-mono text-xs text-emerald-300 focus:outline-none focus:border-amber-500/60"
              />
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                ...
                {executionResult?.status && (
                  <span className="text-[10px] font-mono text-emerald-400">Status: {executionResult.status}</span>
                )}
              </div>
              <pre className="p-4 bg-slate-950 rounded-xl text-xs font-mono text-slate-200 overflow-x-auto border border-slate-800 max-h-80">
                {executionResult
